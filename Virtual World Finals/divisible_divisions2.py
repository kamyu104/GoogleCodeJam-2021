    ...
    for i in xrange(1, len(S)+1):
        ...
        for k in xrange(1, l+1):  # O(logD) times
            ...
            if k == l:
                ...
                break
            if curr2 == 0 and suffix[j%w] == suffix[i%w]:  # (S[j:i]%d_2_5 == 0) and (suffix[j]-suffix[i] == 0 <=> (S[j:i]*10^(len(S)-i)%d_remain == 0 and gcd(d_remain, 10^(len(S)-i)) == 1) <=> S[j:i]%d_remain == 0) <=> S[j:i]%D == 0
